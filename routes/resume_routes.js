    ...
    if(req.query.resume_id == null) {
        ...
        resume_dal.getById(req.query.resume_id, function(err,result) {
           if (err) {
               ...
               for(var i = 0; i < result.length; i++ ){
                   companys[i] = result[i].company_name;
               }

               var unique_schools = school.filter(function (elem, index, self) {
               ...
               var unique_skills = skills.filter(function (elem, index, self) {
                ...
                var unique_companys = companys.filter(function (elem, index, self) {
                    return index == self.indexOf(elem);
                });

               res.render('resume/resumeViewById', {'result': result, "schools": unique_schools, 'skills': unique_skills, 'companys': unique_companys});
           }
        });
    }
});
                                    ...
                                    if (err) {
                                        ...
                                        resume_dal.edit(req.query.resume_id, function (err, result4) {
                                            if (err) {
                                                ...
                                                school = [result4.length];
                                                skills = [result4.length];
                                                companys = [result4.length];

                                                for (var i = 0; i < result4.length; i++) {
                                                    school[i] = result4[i].school_name;
                                                }
                                                ...
                                                for (var i = 0; i < result4.length; i++) {
                                                    companys[i] = result4[i].company_name;
                                                }
